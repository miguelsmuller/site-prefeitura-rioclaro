import { Component } from '@angular/core';

@Component({
  selector: 'app-home',
  templateUrl: './home.component.html',
  styleUrls: ['./home.component.scss'],
})
export class HomeComponent {
  public services = [
    {
      title: 'ISS Eletrônico e Nota Fiscal Eletrônica',
      description: 'Emita e consulte Notas Fiscais Eletrônicas, além de guia de ISS',
      path: '/content',
      icon: `<path d="M0,0h24v24H0V0z" fill="none" />
      <g>
        <path d="M14,9h-4C9.45,9,9,8.55,9,8v0c0-0.55,0.45-1,1-1h4c0.55,0,1,0.45,1,1v0C15,8.55,14.55,9,14,9z" />
        <path
          d="M14,12h-4c-0.55,0-1-0.45-1-1v0c0-0.55,0.45-1,1-1h4c0.55,0,1,0.45,1,1v0C15,11.55,14.55,12,14,12z"
        />
        <path
          d="M19.5,3.5L18,2l-1.5,1.5L15,2l-1.5,1.5L12,2l-1.5,1.5L9,2L7.5,3.5L6,2v14H4c-0.55,0-1,0.45-1,1v2c0,1.66,1.34,3,3,3h12 c1.66,0,3-1.34,3-3V2L19.5,3.5z M15,20H6c-0.55,0-1-0.45-1-1v-1h3h4h3V20z M19,19c0,0.55-0.45,1-1,1s-1-0.45-1-1v-2 c0-0.55-0.45-1-1-1h-2h-2H8V5h11V19z"
        />
        <circle cx="17" cy="8" r="1" />
        <circle cx="17" cy="11" r="1" />
      </g>`,
    },
    {
      title: 'Licitações do Município',
      description: 'Acompanhe a agenda de licitação dos órgãos públicos Municipais de Rio Claro',
      path: '/content',
      icon: `<path d="M0 0h24v24H0V0z" fill="none" />
      <path
        d="M2 21h10c.55 0 1 .45 1 1s-.45 1-1 1H2c-.55 0-1-.45-1-1s.45-1 1-1zM5.24 8.07l2.83-2.83L20.8 17.97c.78.78.78 2.05 0 2.83-.78.78-2.05.78-2.83 0L5.24 8.07zm8.49-5.66l2.83 2.83c.78.78.78 2.05 0 2.83l-1.42 1.42-5.65-5.66 1.41-1.41c.78-.79 2.05-.79 2.83-.01zm-9.9 7.07l5.66 5.66-1.41 1.41c-.78.78-2.05.78-2.83 0l-2.83-2.83c-.78-.78-.78-2.05 0-2.83l1.41-1.41z"
      />`,
    },
    {
      title: 'IPTU, COSIP e Taxas',
      description: 'Retire sua 2ª via de IPTU e todas as cotas, além de alvará de cartão, 2a via de alvará e outros',
      path: '/content',
      icon: `<g>
      <rect fill="none" height="24" width="24" />
      <rect fill="none" height="24" width="24" />
    </g>
    <g>
      <g>
        <path
          d="M20,3h-8c-0.55,0-1,0.45-1,1v1.61c0,0,0,0,0.01,0.01l5,4.5C16.64,10.68,17,11.5,17,12.35V13h2v2h-2v2h2v2h-2v2h3 c0.55,0,1-0.45,1-1V4C21,3.45,20.55,3,20,3z M15,7h-2V5h2V7z M19,11h-2V9h2V11z M19,7h-2V5h2V7z"
        />
        <path
          d="M15,20v-7.65c0-0.28-0.12-0.55-0.33-0.74l-5-4.5C9.48,6.93,9.24,6.85,9,6.85c-0.24,0-0.48,0.09-0.67,0.26l-5,4.5 C3.12,11.79,3,12.06,3,12.35V20c0,0.55,0.45,1,1,1h2c0.55,0,1-0.45,1-1v-4h4v4c0,0.55,0.45,1,1,1h2C14.55,21,15,20.55,15,20z"
        />
      </g>
    </g>`,
    },
    {
      title: 'Parcelamento de Débitos',
      description: 'Opções para emissão de 2a via de documento de arrecadação',
      path: '/content',
      icon: `<g>
      <rect fill="none" height="24" width="24" />
      <path
        d="M14,2H6C4.9,2,4,2.9,4,4v16c0,1.1,0.9,2,2,2h12c1.1,0,2-0.9,2-2V8L14,2z M6,20V4h7v4h5v12H6z M11,19h2v-1h1 c0.55,0,1-0.45,1-1v-3c0-0.55-0.45-1-1-1h-3v-1h4v-2h-2V9h-2v1h-1c-0.55,0-1,0.45-1,1v3c0,0.55,0.45,1,1,1h3v1H9v2h2V19z"
      />
    </g>`,
    },
    {
      title: 'Emissão de Certidões',
      description: 'Certidão Negativa de débitos do imóvel, dados cadastrais do imóvel e outras clicando aqui',
      path: '/content',
      icon: `<path style="fill: none" d="M0,0h24v24H0V0z" />
      <path
        d="M14,2H6C4.9,2,4,2.9,4,4v16c0,1.1,0.9,2,2,2h12c1.1,0,2-0.9,2-2V8L14,2z M18,20H6V4h7v5h5V20z M13,15.8l0.5,1.1l1.2,0.6
        L13.5,18L13,19.1L12.5,18l-1.2-0.5l1.2-0.6L13,15.8z M14.8,11.9l-1.1-0.5l1.1-0.5l0.5-1.2l0.6,1.2l1.1,0.5l-1.1,0.5l-0.6,1.2
        L14.8,11.9z M11,12l2.3,1L11,14l-1,2.3L9,14l-2.3-1L9,12l1-2.3L11,12z"
      />`,
    },
    {
      title: 'Recolhimento de Entulho',
      description: 'A entrega da caçamba é feita de Segunda a Sexta- Feira, clique aqui e agende a sua',
      path: '/content',
      icon: `<path
      stroke-linecap="round"
      stroke-linejoin="round"
      stroke-width="2"
      d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"
    />`,
    },
    {
      title: 'Concursos e Processos Seletivos',
      description: 'Obtenha todas as informações disponíveis sobre o andamento de processos e concursos',
      path: '/content',
      icon: `<rect fill="none" height="24" width="24" />
      <g>
        <path
          d="M22,8c0-0.55-0.45-1-1-1h-7c-0.55,0-1,0.45-1,1s0.45,1,1,1h7C21.55,9,22,8.55,22,8z M13,16c0,0.55,0.45,1,1,1h7 c0.55,0,1-0.45,1-1c0-0.55-0.45-1-1-1h-7C13.45,15,13,15.45,13,16z M10.47,4.63c0.39,0.39,0.39,1.02,0,1.41l-4.23,4.25 c-0.39,0.39-1.02,0.39-1.42,0L2.7,8.16c-0.39-0.39-0.39-1.02,0-1.41c0.39-0.39,1.02-0.39,1.41,0l1.42,1.42l3.54-3.54 C9.45,4.25,10.09,4.25,10.47,4.63z M10.48,12.64c0.39,0.39,0.39,1.02,0,1.41l-4.23,4.25c-0.39,0.39-1.02,0.39-1.42,0L2.7,16.16 c-0.39-0.39-0.39-1.02,0-1.41s1.02-0.39,1.41,0l1.42,1.42l3.54-3.54C9.45,12.25,10.09,12.25,10.48,12.64L10.48,12.64z"
        />
      </g>`,
    },
    {
      title: 'Ouvidoria Online',
      description: 'Solicitações, informações e reclamações de serviços, além de atendimentos de emergência',
      path: '/content',
      icon: `<g>
      <rect fill="none" height="24" width="24" />
      <rect fill="none" height="24" width="24" />
    </g>
    <g>
      <g>
        <path
          d="M21,12.22C21,6.73,16.74,3,12,3c-4.69,0-9,3.65-9,9.28C2.4,12.62,2,13.26,2,14v2c0,1.1,0.9,2,2,2h0c0.55,0,1-0.45,1-1 l0-4.81c0-3.83,2.95-7.18,6.78-7.29c3.96-0.12,7.22,3.06,7.22,7V19h-7c-0.55,0-1,0.45-1,1v0c0,0.55,0.45,1,1,1h7c1.1,0,2-0.9,2-2 v-1.22c0.59-0.31,1-0.92,1-1.64v-2.3C22,13.14,21.59,12.53,21,12.22z"
        />
        <circle cx="9" cy="13" r="1" />
        <circle cx="15" cy="13" r="1" />
        <path
          d="M18,11.03C17.52,8.18,15.04,6,12.05,6c-3.03,0-6.29,2.51-6.03,6.45c2.47-1.01,4.33-3.21,4.86-5.89 C12.19,9.19,14.88,11,18,11.03z"
        />
      </g>
    </g>`,
    },
    {
      title: 'Reparo de Buracos',
      description: 'Certidão Negativa de débitos do imóvel, dados cadastrais do imóvel e outras clicando aqui',
      path: '/content',
      icon: `<path
      stroke-linecap="round"
      stroke-linejoin="round"
      stroke-width="2"
      d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"
    />`,
    },
    {
      title: 'Reparo de Lâmpadas',
      description: 'Certidão Negativa de débitos do imóvel, dados cadastrais do imóvel e outras clicando aqui',
      path: '/content',
      icon: `<g><path d="M0,0h24v24H0V0z" fill="none" /></g>
      <g>
        <g>
          <rect height="3" width="2" x="11" y="19" />
          <rect height="2" width="3" x="2" y="11" />
          <rect height="2" width="3" x="19" y="11" />
          <rect
            height="3"
            transform="matrix(0.7071 -0.7071 0.7071 0.7071 -7.6665 17.8014)"
            width="1.99"
            x="16.66"
            y="16.66"
          />
          <rect
            height="1.99"
            transform="matrix(0.7071 -0.7071 0.7071 0.7071 -10.9791 9.8041)"
            width="3"
            x="4.85"
            y="17.16"
          />
          <path
            d="M15,8.02V3H9v5.02C7.79,8.94,7,10.37,7,12c0,2.76,2.24,5,5,5s5-2.24,5-5C17,10.37,16.21,8.94,15,8.02z M11,5h2v2.1 C12.68,7.04,12.34,7,12,7s-0.68,0.04-1,0.1V5z M12,15c-1.65,0-3-1.35-3-3s1.35-3,3-3c1.65,0,3,1.35,3,3S13.65,15,12,15z"
          />
        </g>
      </g>`,
    },
    {
      title: 'Unidades de Saúde',
      description: 'Certidão Negativa de débitos do imóvel, dados cadastrais do imóvel e outras clicando aqui',
      path: '/content',
      icon: `<path d="M0 0h24v24H0V0z" fill="none" />
      <path
        d="M19 3H5c-1.1 0-1.99.9-1.99 2L3 19c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5V5h14v14zm-8.5-2h3v-3.5H17v-3h-3.5V7h-3v3.5H7v3h3.5z"
      />`,
    },
    {
      title: 'Unidades de Ensino',
      description: 'Certidão Negativa de débitos do imóvel, dados cadastrais do imóvel e outras clicando aqui',
      path: '/content',
      icon: `<path d="M0 0h24v24H0V0z" fill="none" />
      <path
        d="M12 3L1 9l4 2.18v6L12 21l7-3.82v-6l2-1.09V17h2V9L12 3zm6.82 6L12 12.72 5.18 9 12 5.28 18.82 9zM17 15.99l-5 2.73-5-2.73v-3.72L12 15l5-2.73v3.72z"
      />`,
    },
  ];
}
